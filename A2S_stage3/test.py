...
def test_model(model, test_sets, config, epoch=None, saver=None):
    ...
    for set_name, test_set in test_sets.items():
        save_folder = '/data/A2S-USOD/result/cornet/resnet/base/DUTS-TR-Mask-vit_b-50'
        ...
        print('\n Mean-F: {}, MAE: {}, IOU: {:.3f}, dis: {:.3f}.'.format(meanf, mae, np.mean(ious), np.mean(dises)))
        list_file = '/data/A2S-USOD/weight/cornet/resnet/text/vit_b-50.txt'

        with open(list_file, 'a') as f:
            f.write(
                '\n{}/{}, Mean-F: {}, MAE: {}, IOU: {:.3f}, dis: {:.3f}.'.format(j, titer, meanf, mae, np.mean(ious),
                                                                                 np.mean(dises)))
