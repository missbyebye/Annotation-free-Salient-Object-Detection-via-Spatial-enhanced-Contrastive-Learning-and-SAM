...
def get_image_list(name, config, phase):
    ...
    if phase == 'train':
        ...
        gt_root = '/data/dataset/DUTS-TR/DUTS-TR-Mask-vit_b-50'
        gt_root2 = '/data/dataset/DUTS-TR/DUTS-TR-Mask-Mix-base-Deleted-iou95'
        # gt_root = '/data/segment-anything-main/segment-anything-main/figures/DUTS-TR-opt3-box-96'
        # gt_root = '/data/dataset/DUTS-TR/DUTS-TR-Mask-SAMall-iou-95-95'
        # gt_root = '/data/dataset/DUTS-TR/DUTS-TR-Mask-Mix-base-SAM-85-opt3(96-0-0)'
        # merge_folders(gt_root1, gt_root2, gt_root)

        # point和box
        # gts = sorted(
        #     [os.path.join(gt_root, f) for f in os.listdir(gt_root) if f.endswith('point_mask.png') or f.endswith('box_mask.png')])
        gts = sorted(
...
class Train_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.tensor(image).float(), torch.tensor(gt).float()
