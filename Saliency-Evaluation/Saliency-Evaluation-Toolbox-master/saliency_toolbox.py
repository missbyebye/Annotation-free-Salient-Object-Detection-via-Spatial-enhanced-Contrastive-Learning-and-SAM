...
def calculate_measures(gt_dir, sm_dir, measures, save=False, beta=np.sqrt(0.3), gt_threshold=0.5):
    ...
    #
    #     sm_name = sm_name+'.png'
    for sm_name in tqdm(glob(os.path.join(sm_dir, '*'))):
        _, name = os.path.split(sm_name)
        # # 新增-点划分
        ...
        gt_name = os.path.join(gt_dir, name)

        gt_name = gt_name + '.png'

        # print(gt_name)
        # print(sm_name)
        # image = cv2.imread(gt_name, cv2.COLOR_BGR2RGB)
        # if image is None:
        #     print(f"Failed to load image: {gt_name}")
        #     continue
        # image = cv2.imread(sm_name, cv2.COLOR_BGR2RGB)
        # if image is None:
        #     print(f"Failed to load image: {sm_name}")
        #     continue
