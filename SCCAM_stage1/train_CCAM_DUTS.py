...
if __name__ == '__main__':
    ...
    train_dataset = DUTS_Dataset(args.data_dir, train_transform)
    ...
    for epoch in range(args.max_epoch):
        ...
        for iteration, (images, _) in enumerate(train_loader):
            ...
            loss1 = utils.map_spatial_loss(ccam)
            #loss1 = torch.zeros(1)[0].cuda()
            loss2 = criterion[1](bg_feats, fg_feats)
            #oss3 = criterion[2](bg_feats)
            loss3 = torch.zeros(1)[0].cuda()
            ...
            sys.stdout.write('\r# training epoch {}, [{}/{}] = {:.2f}%'.format(epoch, iteration, len(train_loader), (iteration + 1) / len(train_loader) * 100))
            sys.stdout.flush()
            ...
            #################################################################################################
            """
            if (iteration + 1) % (100 // the_number_of_gpu) == 0:
                ...
                data = {
                    ...
                    'loss_aver': loss_sum / (iteration + 1),
                    'loss': loss,
                    'positive_loss': loss1,
                    'negative_loss': loss2,
        ...
        #################################################################################################
        # save_model_fn()
        model_path = model_dir + '{0}_epoch{1}.pth'.format(args.tag, epoch)
