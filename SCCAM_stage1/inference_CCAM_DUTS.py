...
if __name__ == '__main__':
    ...
    dataset = DUTS_Dataset(args.data_dir)
    ...
    def get_cam(ori_image, scale):
        ...
        image = image.resize((round(ori_w*scale), round(ori_h*scale)), resample=PIL.Image.BICUBIC)#LANCZOS)
    ...
    with torch.no_grad():
        ...
        for step, (ori_image, image_id) in enumerate(dataset):
            ...
            cv2.imwrite(f'{cam_path}/{image_id}_map.png', cam.astype(np.uint8))
            cam = colormap(cam)
